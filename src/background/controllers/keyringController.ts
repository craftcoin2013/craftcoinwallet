...
import type { AddressType } from "craftcoinhdw";
import { Network, Psbt } from "craftcoinjs-lib";
import { keyringService } from "../services";
import type { Hex, SendCRC } from "../services/keyring/types";
...
export interface IKeyringController {
  ...
  sendCRC(data: SendCRC): Promise<string>;
...
class KeyringController implements IKeyringController {
   ...
   * @param {SendCRC} data Input data for the transaction
   * @returns {Promise<string>} Hex of transaction to push transaction to the blockchain with
   */
  async sendCRC(data: SendCRC): Promise<string> {
    return await keyringService.sendCRC(data);
  }
